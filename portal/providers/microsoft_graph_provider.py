...
__all__ = ["GraphUserRecord", "MicrosoftGraphProvider"]


class MicrosoftGraphProvider:
    """Fetch users from Microsoft Graph using the SDK-backed MSGraphUsers client."""

    def __init__(self, users_factory: Callable[[], MSGraphUsers] = MSGraphUsers) -> None:
    ...
    async def list_users(self, *, filter_expr: str, select_fields: Optional[list[str]] = None, page_size: int = 999) -> AsyncIterator[GraphUserRecord]:
        ...
        async for record in users.list_users(filter_expr=filter_expr, select_fields=select_fields, page_size=page_size):
            yield record
