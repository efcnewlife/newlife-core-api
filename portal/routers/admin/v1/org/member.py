...
@router.get(path="/pages", status_code=status.HTTP_200_OK, response_model=AdminMemberPersonPages, permissions=[Permission.MEMBER_PERSON.read])
@inject
async def get_member_person_pages(
    query_model: Annotated[GenericQueryBaseModel, Query()], member_person_service: MemberPersonService = Depends(Provide[Container.org_member_person_service])
):
...
@router.post(path="", status_code=status.HTTP_201_CREATED, response_model=UUIDBaseModel, permissions=[Permission.MEMBER_PERSON.create])
@inject
async def create_member_person(
    model: AdminMemberPersonCreate, member_person_service: MemberPersonService = Depends(Provide[Container.org_member_person_service])
):
...
@router.get(path="/{person_id}", status_code=status.HTTP_200_OK, response_model=AdminMemberPersonDetail, permissions=[Permission.MEMBER_PERSON.read])
@inject
...
@router.put(path="/{person_id}", status_code=status.HTTP_204_NO_CONTENT, permissions=[Permission.MEMBER_PERSON.modify])
@inject
async def update_member_person(
    person_id: uuid.UUID, model: AdminMemberPersonUpdate, member_person_service: MemberPersonService = Depends(Provide[Container.org_member_person_service])
):
    await member_person_service.update_person(person_id=person_id, command=update_member_person_to_command(model))


@router.put(path="/{person_id}/link", status_code=status.HTTP_204_NO_CONTENT, permissions=[Permission.MEMBER_PERSON.modify])
@inject
async def link_member_person(
    person_id: uuid.UUID, model: AdminMemberPersonLink, member_person_service: MemberPersonService = Depends(Provide[Container.org_member_person_service])
):
    await member_person_service.link_user(person_id=person_id, command=link_member_person_to_command(model))
