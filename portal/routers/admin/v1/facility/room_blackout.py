...
@router.get(
    ...
    response_model=AdminRoomBlackoutPages,
    permissions=[Permission.FACILITY_ROOM_BLACKOUT.read],
)
@inject
async def get_room_blackout_pages(
    query_model: Annotated[AdminRoomBlackoutQuery, Query()],
    room_blackout_service: RoomBlackoutService = Depends(
        Provide[Container.room_blackout_service]
    ),
):
    command, facility_id = room_blackout_pages_query_to_command(query_model)
    result = await room_blackout_service.get_blackout_pages(
        command=command,
...
@router.get(
    path="/list",
    status_code=status.HTTP_200_OK,
    response_model=AdminRoomBlackoutList,
    permissions=[Permission.FACILITY_ROOM_BLACKOUT.read],
)
@inject
...
@router.post(
    ...
    permissions=[Permission.FACILITY_ROOM_BLACKOUT.create],
)
@inject
async def create_room_blackout(
    model: AdminRoomBlackoutCreate,
    room_blackout_service: RoomBlackoutService = Depends(
        Provide[Container.room_blackout_service]
    ),
):
    result = await room_blackout_service.create_blackout(
        command=create_room_blackout_to_command(model)
    )
...
@router.get(
    path="/{blackout_id}",
    status_code=status.HTTP_200_OK,
    response_model=AdminRoomBlackoutItem,
    permissions=[Permission.FACILITY_ROOM_BLACKOUT.read],
)
@inject
async def get_room_blackout(
...
@router.put(
    ...
    permissions=[Permission.FACILITY_ROOM_BLACKOUT.modify],
)
@inject
async def update_room_blackout(
    blackout_id: uuid.UUID,
    model: AdminRoomBlackoutUpdate,
    room_blackout_service: RoomBlackoutService = Depends(
        Provide[Container.room_blackout_service]
    ),
):
    await room_blackout_service.update_blackout(
        blackout_id=blackout_id,
        command=update_room_blackout_to_command(model),
    )
...
@router.delete(
    path="/{blackout_id}",
    status_code=status.HTTP_204_NO_CONTENT,
    permissions=[Permission.FACILITY_ROOM_BLACKOUT.delete],
)
@inject
async def delete_room_blackout(
    ...
    await room_blackout_service.delete_blackout(
        blackout_id=blackout_id,
...
@router.put(
    path="/{blackout_id}/restore",
    status_code=status.HTTP_204_NO_CONTENT,
    permissions=[Permission.FACILITY_ROOM_BLACKOUT.modify],
)
@inject
async def restore_room_blackout(
    blackout_id: uuid.UUID,
    room_blackout_service: RoomBlackoutService = Depends(
        Provide[Container.room_blackout_service]
    ),
