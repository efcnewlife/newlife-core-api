...
@router.get(path="/policy-settings", status_code=status.HTTP_200_OK, response_model=AdminPolicySettingList, permissions=[Permission.FACILITY_RENTAL_RATE.read])
@inject
...
@inject
async def get_policy_setting(setting_id: uuid.UUID, rental_catalog_service: RentalCatalogService = Depends(Provide[Container.rental_catalog_service])):
    return policy_setting_to_api(await rental_catalog_service.get_policy_setting(setting_id))


@router.put(path="/policy-settings/{setting_id}", status_code=status.HTTP_204_NO_CONTENT, permissions=[Permission.FACILITY_RENTAL_RATE.modify])
@inject
async def update_policy_setting(
    setting_id: uuid.UUID, model: AdminPolicySettingUpdate, rental_catalog_service: RentalCatalogService = Depends(Provide[Container.rental_catalog_service])
):
    await rental_catalog_service.update_policy_setting(setting_id=setting_id, command=update_policy_setting_to_command(model))
