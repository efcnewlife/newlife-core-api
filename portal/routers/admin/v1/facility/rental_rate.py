...
@router.get(path="/pages", status_code=status.HTTP_200_OK, response_model=AdminRentalRatePages, permissions=[Permission.FACILITY_RENTAL_RATE.read])
@inject
async def get_rental_rate_pages(
    query_model: Annotated[AdminRentalRateQuery, Query()], rental_rate_service: RentalRateService = Depends(Provide[Container.rental_rate_service])
):
    command, facility_id = rental_rate_pages_query_to_command(query_model)
    result = await rental_rate_service.get_rate_pages(command=command, facility_id=facility_id)
    return rental_rate_page_to_api(result)


@router.get(path="/list", status_code=status.HTTP_200_OK, response_model=AdminRentalRateList, permissions=[Permission.FACILITY_RENTAL_RATE.read])
@inject
...
@router.post(
    path="/preview-quote", status_code=status.HTTP_200_OK, response_model=AdminPreviewQuoteResponse, permissions=[Permission.FACILITY_RENTAL_RATE.read]
)
@inject
async def preview_quote(model: AdminPreviewQuoteRequest, pricing_service: PricingService = Depends(Provide[Container.pricing_service])):
...
@router.post(path="", status_code=status.HTTP_201_CREATED, response_model=UUIDBaseModel, permissions=[Permission.FACILITY_RENTAL_RATE.create])
@inject
async def create_rental_rate(model: AdminRentalRateCreate, rental_rate_service: RentalRateService = Depends(Provide[Container.rental_rate_service])):
...
@router.get(path="/{rate_id}", status_code=status.HTTP_200_OK, response_model=AdminRentalRateItem, permissions=[Permission.FACILITY_RENTAL_RATE.read])
@inject
...
@router.put(path="/{rate_id}", status_code=status.HTTP_204_NO_CONTENT, permissions=[Permission.FACILITY_RENTAL_RATE.modify])
@inject
async def update_rental_rate(
    rate_id: uuid.UUID, model: AdminRentalRateUpdate, rental_rate_service: RentalRateService = Depends(Provide[Container.rental_rate_service])
):
    await rental_rate_service.update_rate(rate_id=rate_id, command=update_rental_rate_to_command(model))


@router.delete(path="/{rate_id}", status_code=status.HTTP_204_NO_CONTENT, permissions=[Permission.FACILITY_RENTAL_RATE.delete])
@inject
async def delete_rental_rate(
    rate_id: uuid.UUID, model: DeleteBaseModel, rental_rate_service: RentalRateService = Depends(Provide[Container.rental_rate_service])
):
    await rental_rate_service.delete_rate(rate_id=rate_id, command=delete_model_to_command(model))


@router.put(path="/{rate_id}/restore", status_code=status.HTTP_204_NO_CONTENT, permissions=[Permission.FACILITY_RENTAL_RATE.modify])
@inject
async def restore_rental_rate(rate_id: uuid.UUID, rental_rate_service: RentalRateService = Depends(Provide[Container.rental_rate_service])):
