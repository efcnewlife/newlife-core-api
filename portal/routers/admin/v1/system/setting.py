...
@router.get(
    ...
    response_model=AdminSettingList,
    permissions=[Permission.SYSTEM_SETTING.read],
)
@inject
...
async def get_setting_by_key(
    ...
    setting_key: Annotated[str, Query()],
    setting_service: SettingService = Depends(Provide[Container.setting_service]),
):
    result = await setting_service.get_setting_by_key(
        namespace=namespace,
...
@router.post(
    ...
    permissions=[Permission.SYSTEM_SETTING.create],
)
@inject
async def create_setting(
    body: AdminSettingCreate,
    setting_service: SettingService = Depends(Provide[Container.setting_service]),
):
...
@router.put(
    ...
    permissions=[Permission.SYSTEM_SETTING.delete],
)
@inject
async def restore_settings(
    body: AdminSettingBulkAction,
    setting_service: SettingService = Depends(Provide[Container.setting_service]),
):
...
@router.put(
    ...
    status_code=status.HTTP_200_OK,
    response_model=AdminSettingItem,
    permissions=[Permission.SYSTEM_SETTING.modify],
)
@inject
async def update_setting(
    ...
    result = await setting_service.update_setting(
        setting_id=setting_id,
        command=update_setting_to_command(body),
    )
    return setting_result_to_api(result)
...
async def delete_setting(
    setting_id: UUID,
    body: DeleteBaseModel,
    setting_service: SettingService = Depends(Provide[Container.setting_service]),
):
    await setting_service.delete_setting(
        setting_id=setting_id,
        command=delete_model_to_command(body),
    )
