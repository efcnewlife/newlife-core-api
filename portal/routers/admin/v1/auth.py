...
from portal.application.auth.commands import LoginCommand, LoginWithoutValidateCommand, LogoutCommand, MicrosoftLoginCommand, RefreshTokenCommand
...
from portal.serializers.admin.v1.auth import AdminInfo, AdminLoginRequest, AdminLoginResponse, MicrosoftIdTokenRequest
...
@router.post(
    ...
    require_auth=False,
    responses={401: {"description": "Unauthorized", "content": {"application/json": {"example": {"detail": "Invalid authorization token"}}}}},
)
@inject
async def admin_microsoft_login(
    ...
    result = await microsoft_auth_service.microsoft_login(MicrosoftLoginCommand(id_token=body.id_token))
    return login_result_to_api(result)
