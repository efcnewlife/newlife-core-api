...
def _day_to_api(day: DayAvailabilityResult) -> MemberDayAvailability:
    return MemberDayAvailability(
        am=[_slot_to_api(s) for s in day.am],
        pm=[_slot_to_api(s) for s in day.pm],
    )
...
def _availability_list_to_api(result: RoomAvailabilityListResult) -> MemberRoomAvailabilityList:
    return MemberRoomAvailabilityList(
        date=result.date,
        items=[_room_availability_to_api(item) for item in result.items],
    )
...
@router.get(
    ...
    response_model=MemberRoomAvailabilityList,
...
async def get_rooms_availability(
    ...
    result = await availability_service.get_rooms_availability(
        RoomAvailabilityQueryCommand(target_date=date, ministry_id=ministry_id),
    )
...
@router.get(
    path="/bookings/mine",
    status_code=status.HTTP_200_OK,
    response_model=MemberBookingList,
    response_model_by_alias=True,
)
@inject
...
async def cancel_my_booking(
    booking_id: UUID,
    model: MemberBookingCancel,
    booking_service: BookingService = Depends(Provide[Container.booking_service]),
):
    await booking_service.cancel_my_booking(
        booking_id,
        CancelBookingCommand(scope=model.scope, cancel_reason=model.cancel_reason),
    )
