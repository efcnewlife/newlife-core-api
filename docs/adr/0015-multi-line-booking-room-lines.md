...
- At most **3** `booking_room` rows per booking (hard product cap on **line count**, not distinct room count). ADR 0017 supersedes reading this cap from `MAX_ROOMS_PER_BOOKING` Policy Setting; ADR 0018 further supersedes the fixed value itself, making it a `system_setting` (default 10).
