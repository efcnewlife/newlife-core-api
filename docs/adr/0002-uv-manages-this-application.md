...
The `dev` group holds Alembic, Click, and the pytest stack; the production image runs `uv sync --frozen --no-dev --no-install-project` into a project `.venv` on `PATH`. Locally, uv reads `.python-version` (`3.14`) and installs that interpreter if it is missing. The image uses `python:3.14-alpine` and does not download a second interpreter.
