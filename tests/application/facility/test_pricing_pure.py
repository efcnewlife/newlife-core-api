...
from tests.fixtures.facility.factories import make_rental_rate


def test_pick_rate_for_line_uses_daily_flat_at_threshold():
    facility_id = uuid4()
    rates = [
        make_rental_rate(facility_id=facility_id, billing_unit=RentalRateBillingUnit.HOURLY.value),
        make_rental_rate(
    ...
    rate, tier = RentalRepository.pick_rate_for_line(
        ...
        daily_flat_threshold=Decimal("5"),
    )
...
def test_pick_rate_for_line_uses_hourly_below_threshold():
    ...
    rates = [
        make_rental_rate(facility_id=facility_id, billing_unit=RentalRateBillingUnit.HOURLY.value),
        make_rental_rate(facility_id=facility_id, billing_unit=RentalRateBillingUnit.DAILY_FLAT.value),
    ]
    rate, tier = RentalRepository.pick_rate_for_line(
        rates=rates,
