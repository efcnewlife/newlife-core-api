...
class _StubSlotTemplateRepository(StubRoomSlotTemplateRepository):
    ...
    async def list_active_for_day(self, facility_id, day_of_week):
        return [
            ...
            if item.facility_id == facility_id and (item.days_of_week_mask & (1 << day_of_week)) != 0
        ]
...
async def test_availability_excludes_blackout_overlap():
    room_id = uuid4()
    rooms = [
        RoomListItemResult(id=room_id, code="gym", name="Gym", room_number="1", is_active=True),
    ]
    templates = [
        RoomSlotTemplateResult(
            id=uuid4(),
    ...
    result = await service.get_rooms_availability(
        RoomAvailabilityQueryCommand(target_date=date(2026, 7, 20))
    )
